...
The target setup is an **Astra-led Codex PMO workflow**, with `gpt-6-astra` at `high`
as the selected main-agent configuration. PMO describes ownership of scope, integration,
verification, and the final claim; it does not change the user's model or reasoning effort.
These proportionality fixes remain usable by other capable hosts and do not make Astra a
mandatory dependency or claim comparative model performance.

Shell, Git, and test runners do not acquire a model/effort setting from PMO. For model-backed
delegation, use the active host policy and Baton: stable, cheaply falsifiable execution may use
Luna/max; a bounded ordinary independent PMO review may use a distinct Terra/high reviewer when
that host policy admits it. Higher-consequence reviews retain their applicable capability and
independence requirements. Do not silently escalate the model, select a paid provider, or apply
the optional CK / Hermes Prime-minion adapter to Codex. Availability and explicit provider
authorization remain runtime-specific; this document is an applicability note, not a routing override.
